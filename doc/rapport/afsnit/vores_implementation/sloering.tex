...
multiplicere kernel rigtigt på billedet, kaldet Foldning. Udtrak
...
Foldning er en simpel matematisk metode, som beskriver hvordan man kan
gange 2 matricer sammen, i vores tilfælde, en kernel $R^{n}~X~R^{m}$ og
et billedet matrice $R^{k}~X~R^{d}$, hvor $n = d ~\vee~ m = k$ ikke behøver
at være opfyldt.
...
vid forskellige størrelser. Måden Foldning virker på at man
...
Måde Gaussisk sløring bygger sin kernel op på, at ved brug af Normale
...
Hvis $\sigma = 1$ og vi lader x og y køre fra -1 til 1, med skridt størrelse
på 1, og multiplicere alle værdiger, så den laveste værdig kommer til at være 1 og
runder værdien af. Få vi en kernel som se på firgur \ref{gauss}.

\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.5,angle=0]{afsnit/vores_implementation/billeder/sloering/gauss}
	\end{center}
	\caption[]{En $3~X~3$ Kernel for gauss sløring}
	\label{gauss}
\end{figure}
